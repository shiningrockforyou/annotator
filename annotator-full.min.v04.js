...
! function() {
	...
	Annotator = function(_super) {
		...
		Annotator.prototype._setupEditor = function() {
			...
			this.editor.hide().on("hide", this.onEditorHide).on("save", this.onEditorSubmit).addField({
				...
				submit: function(field, annotation) {
					$('.deleteButtonPlace').css('opacity','0')
		...
		Annotator.prototype.checkForEndSelection = function(event) {
			...
			if (event && this.selectedRanges.length) {
				return this.adder.css('position','fixed').show()
			} else {
		...
		Annotator.prototype.onAdderClick = function(event) {
            ...
            console.log('adder click deletebutton??')
			...
			save = function() {
                ...
                $(annotation.highlights).attr('onclick',"$('.annotator-edit').click()");
	...
	Annotator.Viewer = function(_super) {
		...
		Viewer.prototype.onDeleteClick = function(event) {
            let tempCheck = $('.annotator-hl-temporary').length
            if (tempCheck == 0) {
          
            $('.annotator-editor').addClass('annotator-hide')
            $('.deleteButtonPlace').css('opacity','0')
			$('#annotator-field-0').val('')
            console.log('delete click deletebutton??')
